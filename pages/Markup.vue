<template>
  <div>
    <div class="bg-white rounded-md p-4">
      <!-- Update Modal -->
      <UpdateMarkupModal :UpdateVisible="isUpdateModalVisible" @close="hideUpdateModal"  :markup="selectedMarkup" />

      <!-- Popup Modal -->
      <AddMarkupModal :AddVisible="isAddModalVisible" @close="hideAddModal" />
...
import AddMarkupModal from '~/components/AddMarkupModal.vue';
...
// set a delete function to delete markups with id
const deleteMarkup = async (id) => {
  try {
    await axios.delete(`https://example.com/api/markups/${id}`);
    await markupsStore.fetchMarkUps();
    return { success: true, message: 'Markup deleted successfully' };
  } 
  catch (error) {
    console.error('Error deleting markup:', error.response ? error.response.data : error.message);  
